...
# Source SDKMAN (for Maven/Java installed via sdkman)
SDKMAN_INIT="${HOME}/.sdkman/bin/sdkman-init.sh"
if [ ! -f "${SDKMAN_INIT}" ]; then
    # When running under sudo, HOME points to /root — check the real user's home
    REAL_USER_HOME=$(eval echo "~${SUDO_USER:-$USER}")
    SDKMAN_INIT="${REAL_USER_HOME}/.sdkman/bin/sdkman-init.sh"
fi
if [ -f "${SDKMAN_INIT}" ]; then
    export SDKMAN_DIR="$(dirname "$(dirname "${SDKMAN_INIT}")")"
    source "${SDKMAN_INIT}"
fi

# Source nvm (for Node.js/npm installed via nvm)
NVM_INIT="${HOME}/.nvm/nvm.sh"
if [ ! -f "${NVM_INIT}" ]; then
    REAL_USER_HOME=${REAL_USER_HOME:-$(eval echo "~${SUDO_USER:-$USER}")}
    NVM_INIT="${REAL_USER_HOME}/.nvm/nvm.sh"
fi
if [ -f "${NVM_INIT}" ]; then
...
# Use pre-built JAR (skip Maven build entirely)
JAR_SOURCE=""
if [ -f "${CHAINCODE_DIR}/jar_pkg/voting-chaincode-1.0.0.jar" ]; then
    JAR_SOURCE="${CHAINCODE_DIR}/jar_pkg/voting-chaincode-1.0.0.jar"
    print_ok "Using pre-built chaincode JAR from jar_pkg/"
elif [ -f "${CHAINCODE_DIR}/target/voting-chaincode-1.0.0.jar" ]; then
    JAR_SOURCE="${CHAINCODE_DIR}/target/voting-chaincode-1.0.0.jar"
    print_ok "Using Maven-built chaincode JAR from target/"
else
    echo "  Building chaincode with Maven (no pre-built JAR found)..."
    cd "${CHAINCODE_DIR}"
...
if [ "$CC_COMMITTED" -gt 0 ]; then
    ...
    echo "  Preparing chaincode for deployment..."
    rm -rf "${CHAINCODE_DIR}/deploy"
    mkdir -p "${CHAINCODE_DIR}/deploy"
    cp "${JAR_SOURCE}" "${CHAINCODE_DIR}/deploy/"
    ...
    echo "  Installing chaincode on peer (this uploads ~39MB, may take a minute)..."
    ...
    # Smart wait: poll for chaincode container instead of blind sleep
    echo "  Waiting for chaincode container to start (Fabric is building JVM image)..."
    for i in $(seq 1 60); do
        CC_RUNNING=$(docker ps --filter "name=dev-peer" --format "{{.Names}}" 2>/dev/null | head -1)
        if [ -n "$CC_RUNNING" ]; then
            print_ok "Chaincode container ready: ${CC_RUNNING} (${i}s)"
            break
        fi
        printf "  . "
        sleep 2
    ...
    if [ -z "$CC_RUNNING" ]; then
        print_warn "Chaincode container not detected after 120s — may still be building"
    fi
fi
