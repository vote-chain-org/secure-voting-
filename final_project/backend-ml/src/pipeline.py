...
try:
    from src.sdk_matcher import create_template_from_png, create_template_from_raw, get_match_score
    SDK_AVAILABLE = True
...
class VoterDB:
    ...
    def enroll(self, voter_id: str, image_path: str, raw_b64: str = None, width: int = None, height: int = None) -> bool:
        """
        ...
        if SDK_AVAILABLE:
            try:
                if raw_b64:
                    ...
                    sg400 = create_template_from_raw(raw_bytes, width, height)
                    logger.info(f"SDK template created from RAW for {voter_id}")
                else:
                    sg400 = create_template_from_png(image_path)
                    logger.info(f"SDK template created from PNG for {voter_id}")
            except Exception as e:
    ...
    def verify(self, voter_id: str, probe_path: str, threshold=0.4, raw_b64: str = None, width: int = None, height: int = None) -> dict:
        """
        ...
        if is_match and SDK_AVAILABLE and sg400_stored is not None:
            try:
                if raw_b64:
                    raw_bytes = base64.b64decode(raw_b64)
                    probe_template = create_template_from_raw(raw_bytes, width, height)
                else:
...
def verify_voter_for_voting(voter_id, fingerprint_path, db,
                             ...
                             liveness_threshold=0.6, match_threshold=0.4,
                             raw_b64: str = None, width: int = None, height: int = None) -> dict:
    """
    ...
    match_result = db.verify(voter_id, fingerprint_path, match_threshold, 
                             raw_b64=raw_b64, width=width, height=height)
    result.update({
